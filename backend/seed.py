...
def to_decimal(value):
    """Convierte valores a Decimal de forma segura, limpiando casos raros."""
...
def load_data():
    ...
    try:
        df_agrupado = pd.read_csv(DATA_DIR / 'bd_campanias_agrupado.csv')

        df_periodos = pd.read_csv(DATA_DIR / 'bd_campanias_periodos.csv')
        df_periodos['impactos_periodo_vehiculos'] = df_periodos['impactos_periodo_vehículos']

        df_sitios = pd.read_csv(DATA_DIR / 'bd_campanias_sitios.csv')
        ...
        for _, row in df_agrupado.iterrows():
            campaign = Campaign(
                ...
                universo_zona_metro=to_decimal(row['universo_zona_metro']),
                impactos_personas=to_decimal(row['impactos_personas']),
                impactos_vehiculos=to_decimal(row['impactos_vehiculos']),
                frecuencia_calculada=row['frecuencia_calculada'],
                frecuencia_promedio=row['frecuencia_promedio'],
                alcance=to_decimal(row['alcance']),
        ...
        for _, row in df_periodos.iterrows():
            period = CampaignPeriod(
                ...
                period=row['period'],
                impactos_periodo_personas=to_decimal(row['impactos_periodo_personas']),
                impactos_periodo_vehiculos=to_decimal(row['impactos_periodo_vehiculos'])
            )
            db.add(period)
        ...
        for _, row in df_sitios.iterrows():
            site = CampaignSite(
                ...
                impactos_catorcenal=to_decimal(row['impactos_catorcenal']),
                impactos_mensuales=to_decimal(row['impactos_mensuales']),
